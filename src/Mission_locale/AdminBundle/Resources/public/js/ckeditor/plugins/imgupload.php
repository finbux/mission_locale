...
$upload_dir = 'symfony_ml/web/upload/actu/content/';
...
$imgsets = array(
 ...
 'type' => array('gif', 'jpg', 'jpe', 'png')        // allowed extensions
);
...
if(isset($_FILES['upload']) && strlen($_FILES['upload']['name']) > 1) {
  ...
  if(!in_array($type, $imgsets['type'])) $err .= 'Le fichier: '. $_FILES['upload']['name']. ' cette extension n\'est pas autorisée.';
  if($_FILES['upload']['size'] > $imgsets['maxsize']*1000) $err .= '\\n La taille du fichier doit faire aux maximum: '. $imgsets['maxsize']. ' KB.';
  if(isset($width) && isset($height)) {
    if($width > $imgsets['maxwidth'] || $height > $imgsets['maxheight']) $err .= '\\n Width x Height = '. $width .' x '. $height .' \\n La longeur maximum  x Height doit être: '. $imgsets['maxwidth']. ' x '. $imgsets['maxheight'];
    if($width < $imgsets['minwidth'] || $height < $imgsets['minheight']) $err .= '\\n Width x Height = '. $width .' x '. $height .'\\n La longeur minimum x Height doit être: '. $imgsets['minwidth']. ' x '. $imgsets['minheight'];
  }
